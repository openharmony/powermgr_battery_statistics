    ...
    public:
        ...
        void FuzzSubscriber(const uint8_t* data, size_t size) {
            ...
            std::vector<std::string> actions = {
                ...
                std::string(reinterpret_cast<const char*>(data), std::min(size_t(32), size))
            };
            ...
            for (const auto& action : actions) {
                ...
                if (offset + 16 <= size) {
                    std::string strParam(reinterpret_cast<const char*>(&data[offset]),
                                       std::min(size_t(16), size - offset));
